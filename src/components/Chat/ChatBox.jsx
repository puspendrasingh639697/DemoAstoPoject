...
const ChatBox = ({ currentUserId, panditId, panditName, onClose }) => {
    ...
    const showNotification = (msg, type = 'info') => {
        ...
        if (Notification.permission === 'granted' && type === 'message') {
    ...
    const initiateCall = () => {
        ...
        // Emit call signal to pandit
        supabase.from('unified_interactions').insert({
            sender_id: currentUserId,
            receiver_id: panditId,
            action_type: 'call_signal',
            payload: { caller_name: panditName, status: 'ringing' }
        });
        showNotification('📞 Calling Pandit...', 'call');
    };
    ...
    useEffect(() => {
        ...
        const onlineSubscription = supabase
            ...
            .on('postgres_changes', 
                { event: 'UPDATE', schema: 'public', table: 'profiles', filter: `id=eq.${panditId}` },
                (payload) => {
    ...
    return (
        <>
            {/* Notification Toast */}
            {notification && (
                <div className={`fixed top-20 right-5 px-4 py-2 rounded-lg shadow-lg z-50 animate-bounce ${
                    notification.type === 'call' ? 'bg-green-500' : 'bg-blue-500'
                } text-white`}>
                    {notification.msg}
                </div>
            )}
            ...
            {/* Chat Window */}
            <div className="fixed bottom-0 right-0 w-full sm:w-96 h-[600px] sm:h-[550px] bg-white rounded-t-2xl shadow-2xl flex flex-col z-50 border border-gray-200" style={{ zIndex: 1000 }}>
                {/* Header */}
                <div className="bg-gradient-to-r from-yellow-500 to-orange-500 p-4 rounded-t-2xl flex justify-between items-center cursor-pointer">
                    <div>
                        <span className="text-white font-bold text-lg">
                            🙏 {panditName}
                            {isTyping && <span className="text-xs text-yellow-200 ml-2 animate-pulse">✏️ typing...</span>}
                    ...
                    <div className="flex gap-2">
                        <button 
                            ...
                            className="bg-green-600 text-white px-3 py-1 rounded-lg text-sm hover:bg-green-700 transition shadow-md"
                        >
                            📞 Call
                        </button>
                        <button onClick={onClose} className="text-white text-2xl hover:text-gray-200">×</button>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-gray-50 to-white">
                    {messages.length === 0 ? (
                        <div className="text-center text-gray-400 py-10">
                            ...
                            No messages yet
                            <p className="text-xs mt-2">Start a conversation with {panditName}!</p>
                        </div>
                    ) : (
                ...
                <div className="p-3 border-t bg-white flex gap-2 rounded-b-2xl">
                    <input 
                        ...
                        onChange={handleTyping}
                        onKeyDown={(e) => e.key === 'Enter' && sendMessage()} 
                        className="flex-1 p-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition" 
                        placeholder="Type a message..." 
                    />
                    <button 
                        onClick={sendMessage} 
                        disabled={!newMessage.trim()}
                        className="px-6 py-3 rounded-xl font-bold bg-gradient-to-r from-yellow-500 to-orange-500 text-white hover:shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Send
                    </button>
                </div>
