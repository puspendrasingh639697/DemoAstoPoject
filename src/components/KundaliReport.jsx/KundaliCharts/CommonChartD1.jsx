...
export default function CommonChartD1({ data, defaultPlanet, defaultHouse,degreeRequired}) {
    ...
    useEffect(() => {
        if (data[defaultPlanetName] && data[defaultPlanetName][defaultHouseNumber]) {
            let d1 = data[defaultPlanetName][defaultHouseNumber];
            const nums = [];
            for (let i = 1; i <= 12; i++) {
                if (d1 > 12) d1 = 1;
                nums.push(d1);
    ...
    useEffect(() => {
        ...
        const drawHouseText = (houseNum, x, y) => {
            ...
            entries.forEach((entry, idx) => {
                ...
                ctx.fillStyle = color;
                ctx.fillText(entry.text, x, y + idx * 12); // 12px spacing
            });
        };
    ...
    }, [kundaliNumbers, houseData,language]);
