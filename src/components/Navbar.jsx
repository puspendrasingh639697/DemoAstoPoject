...
// import PanditNotification from "./PanditNotification"; // 👈 1. Notification File Import ki

const Navbar = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { isLoggedIn, logoutUser, user, isUserLoading, setIsLoggedIn, setUser } =
    useContext(astroContext);
  ...
  // Google Translate Logic (Existing)
  useEffect(() => {
    if (!document.querySelector("#google-translate-script")) {
      const script = document.createElement("script");
      script.id = "google-translate-script";
  ...
  const handleLogout = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}api/logoutv2`, { withCredentials: true });
      if (res.status === 200) {
        setUser(null);
        setIsLoggedIn(false);
        sessionStorage.removeItem("user");
  ...
  const renderNavItem = (to, label, isNew = false) => (
    <li className="relative">
      {isNew && (
        <span className="absolute -top-5 -right-4 bg-yellow-500 text-[10px] text-black px-1.5 rounded-full">new</span>
      )}
      <NavLink
        to={to}
        className={({ isActive }) => isActive ? "text-yellow-500 font-bold" : "hover:text-yellow-500"}
        onClick={() => setMenuOpen(false)}
      >
        {label}
      </NavLink>
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all bg-white shadow text-black`}>
      <div className="flex justify-between items-center px-6 py-3">
        <Link to="/">
          <img src={logo} alt="Astro Captain Logo" className="h-[50px]" />
        </Link>
        ...
        <ul className="hidden xl-nav:flex space-x-[16px] items-center text-sm">
          ...
          {/* Dropdowns (Kundali & Numerology) - Keeping your original logic */}
          <li className="relative">
             <button className={`flex items-center ${isKundaliRoute ? "text-yellow-500 font-semibold" : "hover:text-yellow-500"}`}
              onClick={() => setDropdownOpen(!dropdownOpen)}>
              Kundali <img src={down2} alt="dropdown" className="ml-2 w-3" />
            </button>
            {dropdownOpen && (
              <ul className="absolute top-full left-1/2 -translate-x-1/2 bg-white shadow-lg rounded py-2 w-48">
                <li><NavLink to="/free-kundali" className="block px-4 py-2 hover:bg-yellow-100">Free Kundali</NavLink></li>
                <li><NavLink to="/kundali-matching" className="block px-4 py-2 hover:bg-yellow-100">Kundali Matching</NavLink></li>
              </ul>
            )}
          </li>
          ...
          <button><div id="google_translate_element"></div></button>
        </ul>

        {/* User Actions & Notification */}
        <div className="flex flex-row gap-4 items-center">
          
          {/* ✨ 2. PANDIT NOTIFICATION BELL ✨ */}
          {isLoggedIn && user && (
            <PanditNotification panditId={user.phone} />
          )}

          {isUserLoading ? (
            <div className="w-8 h-8 rounded-full border-2 border-yellow-400 border-b-transparent animate-spin"></div>
          ) : isLoggedIn ? (
            <button onClick={() => navigate("/user-dashboard")}>
              <img
                src={user?.profile_image?.profileImage || loginicon}
                alt="profile"
                className="rounded-full w-9 h-9 border-2 border-yellow-400 object-cover"
              />
            </button>
          ) : (
            <button
              onClick={() => navigate("/login")}
              className="text-sm px-5 py-2 rounded-full bg-yellow-400 text-black font-bold hidden xl-nav:flex"
            >
              Login
            </button>
          )}
          ...
          {/* Mobile Toggle */}
          <button onClick={() => setMenuOpen(!menuOpen)} className="xl-nav:hidden text-2xl">☰</button>
        </div>
      </div>

      {/* Mobile Menu (Notification added here too) */}
      {menuOpen && (
        <div className="xl-nav:hidden fixed top-0 left-0 w-full h-screen bg-white z-50 p-6 overflow-auto">
          <div className="flex justify-between items-center mb-6">
            <Link to="/" onClick={() => setMenuOpen(false)}>
          ...
          <ul className="space-y-5 text-lg font-semibold">
            ...
            {isLoggedIn && (
               <li className="pt-4 border-t">
                  <p className="text-sm text-gray-400 mb-2">Your Notifications</p>
                  <PanditNotification panditId={user.phone} />
               </li>
            )}
          </ul>
