...
const Profile = () => {
  const { user, fetchUserData, setUser } = useContext(astroContext);
  const [editMode, setEditMode] = useState(false);
  const [loading, setLoading] = useState(false);
        ...
        <div>
          {editMode ? (
            <div className="flex gap-2">
              <button className="text-red-600 text-2xl" onClick={() => setEditMode(false)}>
                <ImCross />
              </button>
              <button className="text-green-600 text-2xl" onClick={handleEditSave}>
                <FaCheck />
              </button>
            </div>
          ) : (
            <button className="text-red-600 text-3xl" onClick={() => setEditMode(true)}>
              <BiSolidEditAlt />
            </button>
          )}
        </div>
