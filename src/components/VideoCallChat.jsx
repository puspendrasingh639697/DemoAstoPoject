...
const VideoCallChat = ({ currentUserId, targetUserId, targetName, onClose, isInitiator = false }) => {
    ...
    const startCall = async () => {
        ...
        // ✅ ICE Candidate Handler - User Side
        peerConnection.current.onicecandidate = (event) => {
            if (event.candidate) {
                console.log('📡 ICE Candidate (User):', event.candidate);
                if (socket && socket.connected) {
    ...
    const acceptCall = async (signal) => {
        ...
        // ✅ ICE Candidate Handler - Pandit Side
        peerConnection.current.onicecandidate = (event) => {
            if (event.candidate) {
                console.log('📡 ICE Candidate (Pandit):', event.candidate);
                if (socket && socket.connected) {
                    socket.emit('ice-candidate', {
                        to: targetUserId,
                        candidate: event.candidate
